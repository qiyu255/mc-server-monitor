...
async function resolveServerRealAddress(host, port) {
  let realHost = host;
  let realPort = port;
  let resolvedIp = host;
...
async function pingServer(serverAddr) {
  ...
  try {
    ...
    const pingOptions = { timeout: PING_TIMEOUT };
    if (port !== 25565) {
...
function ServerRow({ result }) {
  ...
  const motdSegments = isOnline ? parseMotd(result.motd) : [];
...
function App({ servers }) {
  ...
  const checkAll = useCallback(async () => {
    ...
    const promises = servers.map(addr => pingServer(addr));
